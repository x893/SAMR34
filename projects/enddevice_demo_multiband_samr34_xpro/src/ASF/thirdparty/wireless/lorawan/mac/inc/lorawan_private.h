...
typedef struct
{
    ...
    uint8_t networkSessionKey[LORAWAN_SESSIONKEY_LENGTH];
    uint8_t applicationSessionKey[LORAWAN_SESSIONKEY_LENGTH];
...
typedef union
{
    ...
    struct
    {
       ...
       uint16_t applicationEui:1;
       uint16_t deviceAddress: 1;
       uint16_t applicationKey:1;
       uint16_t networkSessionKey:1;
       uint16_t applicationSessionKey:1;
	   uint16_t mcastDeviceAddress: 1;
	   uint16_t mcastNetworkSessionKey: 1;
	   uint16_t mcastApplicationSessionKey: 1;
    };
	COMPILER_PACK_RESET()
} LorawanMacKeys_t;
COMPILER_PACK_SET(1)
...
typedef struct _LorawanMcastParams_t
{
	...
	LorawanMcastActivationParams_t activationParams;
...
// Callback functions
void LorawanReceiveWindow1Callback (uint8_t param);

void LorawanReceiveWindow2Callback (uint8_t param);

void LorawanLinkCheckCallback (uint8_t param);
...
void EncryptFRMPayload (uint8_t* buffer, uint8_t bufferLength, uint8_t dir, uint32_t frameCounter, uint8_t* key, uint8_t macBufferIndex, uint8_t* bufferToBeEncrypted, uint32_t devAddr);
...
void LorawanClasscReceiveWindow1Callback(void);
